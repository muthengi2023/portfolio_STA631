...
𝛽0(Intercept) = 0.6827482
β 1(add_to_cart_order) = -0.0380813
Thus, the equation becomes:

logit(𝑝^) = 0.6827482−0.0380813×add_to_cart_order
...
Slope (𝛽1): This represents the change in the log-odds of the outcome for a one-unit increase in the predictor. Specifically, for each additional position in the order that a product is added to the cart, the log-odds of the product being reordered decreases by 0.0380813.
...
𝑝^ =1/1+e −(0.6827482−0.0380813×add_to_cart_order)
​
...
Interpretation:
Intercept: The estimate for the intercept is 0.6848. This represents the log-odds of the reordered variable when add_to_cart_order is zero. Since this is a logistic regression, the exponentiated value of the intercept (exp(0.6848)) would give the odds of reordering when add_to_cart_order is zero.
add_to_cart_order:

The estimate for add_to_cart_order is -0.03812. This negative value indicates that for each one-unit increase in the add_to_cart_order variable, the log-odds of reordering decreases by 0.03812. In other words, the later an item is added to the cart, the less likely it is to be reordered.
